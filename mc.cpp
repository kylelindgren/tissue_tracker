...
//    std::cout << P_L_ << std::endl;
    S_L  = M_L*P_L_*M_L.t() + R_L;
//    std::cout << S_L << std::endl;
    K_L  = P_L_*M_L.t()*S_L.inv();
...
//    std::cout << Xk_L << std::endl;
    ...
    for (int i = 0; i < 2*CP_NUM; i++) {
        Z_L->at<float>(i, 0) = Xk_L.at<float>(i*2, 0);
        Z_R->at<float>(i, 0) = Xk_R.at<float>(i*2, 0);
    }
